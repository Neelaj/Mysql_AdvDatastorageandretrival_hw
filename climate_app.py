...
#Create references to Measurements and Stations tables
...
def homepage():
    ...
    return(
        ...
        f"Most recent available date is 2017-08-23 while the latest is 2010-01-01.<br/>"

        f"/api/v1.0/precipitation<br/>"
        f"- Query result dates and temperature from the last year. <br/>"

        f"/api/v1.0/stations<br/>"
        f"- list of stations. <br/>"

        f"/api/v1.0/tobs<br/>"
        f"- list of Temperature Observations tobs for previous year. <br/>"

        f"/api/v1.0/yyyy-mm-dd/<br/>"
        f"- Average, Max, and Min temperature for given date.<br/>"

        f"/api/v1.0/yyyy-mm-dd/yyyy-mm-dd/<br/>"
        f"- Aveage Max, and Min temperature for given period.<br/>"
    )
...
def precipitation():
    ...
    results = session.query(Measurements.date, Measurements.tobs).\
        filter(Measurements.date <= "2017-01-01", Measurements.date >= "2017-01-01").\
        all()
...
def stations():
    """ list of stations"""
    results = session.query(Stations.name, Stations.station, Stations.elevation).all()
...
def given_date(date):
    """average temp, max temp, and min temp for the date"""
...
def query_dates(start_date, end_date):
    """ avg, max, min, temp over a specific time period"""
